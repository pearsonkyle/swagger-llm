...
- URLs for JS/CSS resources are injected via template parameters; internal DocBuddy module paths are hardcoded in the template.
- `debug=True` enables auto-reload for development.
- FOUC fix: Theme injection script runs immediately in `<head>`.

## Provider Presets

Defined in HTML template and `core.js`:
- Ollama (`http://localhost:11434/v1`)
